...
MUTED = "#888888"
INK = "#ffffff"
KEY = "#ffffff"      # orange keys (matches Andrew)
SECTION = "#ffffff"  # blue section headers
GREEN = "#ffffff"
ACCENT = "#ffffff"
...
parts = [
    ...
    f'font-family="Geist Mono, ui-monospace, SFMono-Regular, Menlo, Consolas, monospace">',
    '<defs>'
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        ...
        txt = esc(row[1])
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" fill="{INK}" font-size="12.5">- {txt}</text>')
    else:
        continue
